...
if($_GET['delid']!="")
{
 $seld="select * from slider where id='".$_GET['delid']."'";
 $exed=mysqli_query($link,$seld) or die("select query fail..");  ;
 $fetchd=mysqli_fetch_array($exed, MYSQLI_ASSOC);
 $pathd="../slider_image/".$fetchd['slimg'];
 unlink($pathd);


 $del="delete from slider where id='".$_GET['delid']."'";
...
if($_POST['delete']=="Delete")
{
 ...
 for($a=0;$a<$tot;$a++)
 {
  $seld="select * from slider where id='".$_POST['chk'][$a]."'";
  $exed=mysqli_query($link,$seld) or die("select query fail..");  ;
  $fetchd=mysqli_fetch_array($exed, MYSQLI_ASSOC);
  $pathd="../slider_image/".$fetchd['slimg'];
  unlink($pathd);
  ...
  $mdel="delete from slider where id='".$_POST['chk'][$a]."'"; 
...
$selgal="select * from slider order by id ASC  limit $start, $end";
...
<div class="container">   <!--container-->
  <div class="ctop">View Slider</div>   <!--ctop-->
  <form method="post">
    <div class="cmid">    <!--cmid-->
      ...
      <div class="viewsun">   <!--viewsun-->
        ...
        <div style="width:172px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
          <div><input type="text" name="sltitle" class="w1602" value="<?php echo $fetchgal['sltitle'];?>" /></div>
        </div>
        <div style="width:172px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
          <div><img src="../slider_image/<?php echo $fetchgal['slimg'];?>" height="100%" class="w1601" /></div>
        </div>
        <div style="width:132px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
          <div><input type="text" name="date" class="w1201" value="<?php echo $fetchgal['date'];?>" /></div>
        </div>
        <div style="width:112px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
          <div><input type="text" name="visible." class="w1003" value="<?php if($fetchgal['slvisible']==1) {echo"Show";} else echo "Hide"; ?>" ></div>
        </div>
        <div style="width:162px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
          <div>
            <div class="w16842">
              <a href="addslider.php?menu_id=4&updid=<?php echo $fetchgal['id']; ?>"><div style="float:left; margin-left:20px; color:#fff;">Edit</div></i></a>
            </div>
            <div class="w16842">
              <a href="viewslider.php?delid=<?php echo $fetchgal['id'];?>" onclick="return del();"><div style="float:left; margin-left:10px; color:#fff;">Delete</div></a>
            </div>
          </div>
        </div>
      </div>    <!--/viewsun-->
...
?>
      <a href="viewslider.php?page=0">First</a>
      <a href="viewslider.php?page=<?php echo $prev;?>">Prev</a>
        <?php } ?>
              
      <?php
      $sel_pag="select * from slider";
      ...
      {
      ?>
      <a href="viewslider.php?page=<?php echo $a;?>"> <span class="circle"><?php echo $a+1;?></span></a>
      <?php
      }
      ?>
  ...
  ?>
      ...
      <a href="viewslider.php?page=<?php echo $next;?>">Next</a>
      <a href="viewslider.php?page=<?php echo $final_tot-1;?>">Last</a>
