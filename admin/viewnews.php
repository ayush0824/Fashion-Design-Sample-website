...
if($_GET['delid']!="")
{
 $seld="select * from news where id='".$_GET['delid']."'";
 $exed=mysqli_query($link,$seld) or die("select query fail..");  ;
 $fetchd=mysqli_fetch_array($exed, MYSQLI_ASSOC);
 $pathd="../news_image/".$fetchd['nimg'];
 unlink($pathd);



 $del="delete from news where id='".$_GET['delid']."'";
...
if($_POST['delete']=="Delete")
{
 ...
 for($a=0;$a<$tot;$a++)
 {
  $seld="select * from news where id='".$_POST['chk'][$a]."'";
  $exed=mysqli_query($link,$seld) or die("select query fail..");  ;
  $fetchd=mysqli_fetch_array($exed, MYSQLI_ASSOC);
  $pathd="../news_image/".$fetchd['nimg'];
  unlink($pathd);

  $mdel="delete from news where id='".$_POST['chk'][$a]."'"; 
  mysqli_query($link,$mdel) or die("Multiple delete fail");
 }
}


$selgal="select * from news order by id ASC  limit $start, $end";
$exegal=mysqli_query($link, $selgal)or die("news selection failed");
$isshow=mysqli_num_rows($exegal);

include "header.php";
?>
<div class="container">		<!--container-->
	<div class="ctop">View News</div>		<!--ctop-->
  <form method="post">
    <div class="cmid">		<!--cmid-->
      ...
      <div class="viewsun">		<!--viewsun-->
      	<div style="width:132px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
      		<div style="width:120px; height:38px; border:1px solid #444; margin:5px 5px 0px 5px;">
          <input type="checkbox" class="w1202" name="chk[]"  value="<?php echo $fetchgal['id'];?>" /></div>
        </div>
        <div style="width:113px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
        	<div><input type="text" name="s.no." class="w1003" value="<?php echo $sno++ ;?>" /></div>
        </div>
        <div style="width:172px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
  	      <div><input type="text" name="imgtitle" class="w1602" value="<?php echo $fetchgal['ntitle'];?>" /></div>
        </div>
        <div style="width:172px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
  	      <div><img src="../news_image/<?php echo $fetchgal['nimg'];?>" width="100%" height="100%" class="w1601" /></div>
        </div>
        <div style="width:132px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
  	      <div><input type="text" name="date" class="w1201" value="<?php echo $fetchgal['date'];?>" /></div>
        </div>
        <div style="width:112px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
  	      <div><input type="text" name="visible." class="w1003" value="<?php if($fetchgal['nvisible']==1) {echo"Show";} else echo "Hide"; ?>" /></div>
        </div>
        <div style="width:162px; height:50px; border:1px solid #444; float:left; margin-left:2px;">
  	      <div>
          	<div class="w16842">
          		<a href="addnews.php?menu_id=6&updid=<?php echo $fetchgal['id']; ?>"><div style="float:left; margin-left:20px; color:#fff;">Edit</div></i></a>
            </div>
            <div class="w16842">
            	<a href="viewnews.php?delid=<?php echo $fetchgal['id'];?>" onclick="return del();"><div style="float:left; margin-left:10px; color:#fff;">Delete</div></a>
            </div>
          </div>
        </div>
      </div>		<!--/viewsun-->
...
?>
      <a href="viewnews.php?page=0">First</a>
      <a href="viewnews.php?page=<?php echo $prev;?>">Prev</a>
        <?php } ?>
              
      <?php
      $sel_pag="select * from news";
      ...
      {
      ?>
      <a href="viewnews.php?page=<?php echo $a;?>"> <span class="circle"><?php echo $a+1;?></span></a>
      <?php
      }
      ?>
  ...
  ?>
      ...
      <a href="viewnews.php?page=<?php echo $next;?>">Next</a>
      <a href="viewnews.php?page=<?php echo $final_tot-1;?>">Last</a>
